...
from utils.paper_utils import pull_hf_daily
...
def daily_feed():
    ...
    hf_link, papers = pull_hf_daily()

    last_week = (datetime.now() - timedelta(days=7)).strftime("%Y-%m-%d")
    message = f"_*Top 5 papers at <{hf_link}|{last_week}>*_"
    ...
    for paper in papers:
        ...
        categories = paper["categories"] if "categories" in paper else []

        feed_paper(client, feed_channel, arxiv_id, link, title, abstract, dp_path, categories)
